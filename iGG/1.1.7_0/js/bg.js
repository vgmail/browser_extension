...
var mzk_config = {
    ...
    'notice' : {title:"购买VIP可加速油管等更多网站",url:"login.html"},
...
function KeepLive_Session() {
    ...
    MZK_getJSON_DATA("chromeext/index/getsession/token/"+mzk_user_token+"?appver=" + mzk_config.app_ver, {geoip_info:ipdata}, function (data) {
        if (data.result == "ok") {
            ...
            if(data.auto_test_server.length > 2 && mzk_select_server_info.line_mode == "smart") { // 处理测速服务器数据
...
console.log("Hi , guys 欢迎你抱着学习交流的目的研究本插件。也请尊重自己的行业和劳动成果,假如人人都用破解软件那么我们的行业和你的价值也就没有了存在的必要。");
